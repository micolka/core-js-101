...
function getPolynom(...args) {
  ...
  return (x) => argArr.reduce((acc, el, i) => el * (x ** i) + acc, 0);
}
...
function logger(func, logFunc) {
  return (...args) => {
    let funArgs = JSON.stringify(args);
    funArgs = funArgs.substr(1, funArgs.length - 2);
    let log = `${func.name}(${funArgs}) starts`;
    logFunc(log);
    const result = func(...args);
    log = `${func.name}(${funArgs}) ends`;
    logFunc(log);
    return result;
  };
}
